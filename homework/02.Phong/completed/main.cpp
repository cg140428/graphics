...
kmuvcl::math::vec3f     cam_position(0.0f, 10.0f, 50.0f);
kmuvcl::math::vec3f     light_position(0.0f, 10.0f, 50.0f);
kmuvcl::math::vec4f     light_diffuse(1.0f, 1.0f, 1.0f, 1.0f);
kmuvcl::math::vec4f     material_diffuse(0.88f, 0.31f, 0.55f, 1.0f);

kmuvcl::math::vec4f     light_ambient(0.3f, 0.3f, 0.3f, 1.0f);
kmuvcl::math::vec4f     light_specular(1.0f, 1.0f, 1.0f, 1.0f);
kmuvcl::math::vec4f     material_ambient(0.4f, 0.5f, 0.1f, 1.0f);
kmuvcl::math::vec4f     material_specular(1.0f, 1.0f, 1.0f, 1.0f);
...
void init_shader_program()
{
	...
	loc_u_PVM = glGetUniformLocation(program, "u_pvm_matrix");
	

	loc_a_position = glGetAttribLocation(program, "a_vertex");
	loc_a_normal = glGetAttribLocation(program, "a_normal");


	loc_u_M   = glGetUniformLocation(program, "u_normal_matrix");
	loc_u_VM = glGetUniformLocation(program,"u_vm_matrix");

	loc_u_camposition	= glGetUniformLocation(program, "u_cam_verctor_vc");	
	loc_u_light_position   = glGetUniformLocation(program, "u_light_vector_vc");
...
void key_callback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_UP && action == GLFW_PRESS)
		light_position += kmuvcl::math::vec3f(0.0f, 5.0f, 0.0f);
	if (key == GLFW_KEY_DOWN && action == GLFW_PRESS)
		light_position -= kmuvcl::math::vec3f(0.0f, 5.0f, 0.0f);
	if (key == GLFW_KEY_LEFT && action == GLFW_PRESS)
		light_position -= kmuvcl::math::vec3f(5.0f, 0.0f, 0.0f);
...
void render_object()
{
	...
	glUniformMatrix4fv(loc_u_VM, 1, GL_FALSE, mat_view*mat_model);

	glUniform3fv(loc_u_camposition, 1, camera.position());
